...
async function fetchUserChangePassword(token, phoneNumber, newPassword) {
  ...
  return await axios.put(
    `https://localhost:7018/api/ManagerAccount/ChangePasswordAdmin?token=${token}`,
    requestBody
  );
}
...
async function fetchUserProfile(token, phoneNumber) {
  return await axios.get(
    `https://localhost:7018/api/User/GetProfile?token=${token}&phoneNumber=${phoneNumber}`
  );
}

async function fetchUserRestore(phoneNumber) {
  return await axios.put(
    `https://localhost:7018/api/ManagerAccount/UnLockAccount?phoneNumber=${phoneNumber}`
  );
}

async function fetchUserLock(phoneNumber) {
  return await axios.put(
    `https://localhost:7018/api/ManagerAccount/LockAccount?phoneNumber=${phoneNumber}`
  );
}
...
async function fetchUserDeleteData(token, phoneNumber) {
  return await axios.delete(
    `https://localhost:7018/api/ManagerCustomer/DeleteCustomer?token=${token}&phoneNumber=${phoneNumber}`
  );
}

async function fetchUserSearchData(token, textInput, pageIndex, pageSize) {
  return await axios.get(
    `https://localhost:7018/api/ManagerCustomer/SearchCustomers?token=${token}&textInput=${textInput}&pageIndex=${pageIndex}&pageSize=${pageSize}`
  );
}

async function fetchUserData(token, status, gender, pageIndex, pageSize) {
  return await axios.get(
    `https://localhost:7018/api/ManagerCustomer/GetAllCustomers?token=${token}&status=${status}&gender=${gender}&pageIndex=${pageIndex}&pageSize=${pageSize}`
  );
}
...
function renderUserPage(data) {
  ...
  data.listCustomerInfor.forEach((user) => {
    const row = `
        ...
        : user.status === -1
          ? `<i class="fa-solid fa-lock text-danger" onclick="unLockUser('${user.phoneNumber}')"></i>`
          : user.status === -2
            ? `<i class="fa-solid fa-square-minus" id="restoreUserIcon" onclick="restoreUser('${user.phoneNumber}')"></i>`
            : ""
      }
                </td>
                <td class="popup_trigger--user" onclick="togglePopup(event,'popup_content--user')">
                    ...
                    <div class="popup_content--user d-flex d-none flex-column">
                        <a href="#" class="popup-option" onclick="detailUser(event,${user.phoneNumber
      })">Detail</a>
                        ${user.status === -2
        ? ""
        : `<a href="#" class="popup-option" onclick="deleteUser(event,${user.phoneNumber})">Delete</a>`
      }
                    </div>
                </td>
            </tr>`;
...
function renderUserPagination(currentUserPage, totalPage, paginationLoad) {
  ...
  var htmlContent = "";
  let onClickFunction =
    localStorage.getItem("searchUserStatus") == "True"
      ? "searchUser"
      : "handleUsersPage";
  ...
  htmlContent += `<li class="page_item--user">
                    ...
                    <li class="page_item--user">
                        <a class="page-link" href="#" onclick="periousUserPage(${lastUserPage
      ? currentUserPage - 4 - residual
      : currentUserPage - 4
    })">
  ...
  for (
    let page = lastUserPage ? totalPage - residual : currentUserPage;
    page <= totalPage && page < currentUserPage + 4;
    page++
  ) {
  ...
  htmlContent += `<li class="page_item--user">
                        <a class="page-link" href="#" onclick="nextUserPage(${currentUserPage + 4
    },${totalPage})">
  ...
  localStorage.setItem(
    "currentUserPage",
    lastUserPage ? currentUserPage - residual : currentUserPage
  );
}
...
document.addEventListener("click", function (event) {
  ...
  if (
    !clickedPopup ||
    !clickedPopup.classList.contains("popup_content--user")
  ) {
  ...
  if (
    !clickedPopup ||
    !clickedPopup.classList.contains("popup_content--product")
  ) {
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-10 px-xl-10">
                                ...
                                <div class=" py-1-9 px-1-9 px-sm-6 mb-1-9 rounded">
                                    <h3 class="h2 mb-0">${data.fullName
    }</h3>                          
                                </div>
                                <ul class="list-unstyled mb-1-9">
                                    <li class="mb-2 mb-xl-3 display-28"><span class="display-26 text-secondary me-2 font-weight-600">Phone number:</span> ${data.phoneNumber
    }</li>
                                    <li class="mb-2 mb-xl-3 display-28"><span class="display-26 text-secondary me-2 font-weight-600">Date of birth:</span>${new Date(
      data.dob
    ).toLocaleDateString()}</li>
                                    <li class="mb-2 mb-xl-3 display-28"><span class="display-26 text-secondary me-2 font-weight-600">Gender:</span>${data.gender === 0 ? "Nam" : "Nữ"
    }</li>
                                    <li class="mb-2 mb-xl-3 display-28"><span class="display-26 text-secondary me-2 font-weight-600">Status:</span>${data.status === 0
      ? "Unlock"
      : data.status === -1
        ? "Lock"
        : data.status === -2
          ...
          : ""
                                    ...
                                    ${data.status === -2
      ? ""
      : `<li class="mb-2 mb-xl-3 display-28"><span class="display-26 text-secondary me-2 font-weight-600">New password:</span> 
                                ...
                                <div class="button_profile d-flex justify-content-end mx-2">
                                    ...
                                    <button type="button" class="btn btn-success" onclick="${data.status === -2
      ? `restoreUser(${data.phoneNumber})`
      : `savePassword(${data.phoneNumber})`
    }">${data.status === -2 ? "Restore" : "Save"
    }</button>
                                </div>
                            </div>
                        </div>
...
function savePassword(phoneNumber) {
  ...
  if (newPassword !== repeatPassword) {
    ...
    fetchUserChangePassword(token, phoneNumber, newPassword).catch((error) =>
...
function fetchAdminData(token) {
  return axios.get(
    `https://localhost:7018/api/ManagerAccount/GetAdminInfor?token=${token}`
  );
}
  ...
  tableBody.innerHTML += `
        <div class="profile">
            ...
            <div class="profile-photo">
                <img src="../assets/images/admin/avatar_admin.jpg">
            </div>
        </div>
    `;
}
...
async function fetchProductSeachData(nameProduct, pageIndex, pageSize) {
  return await axios.post(
    `https://localhost:7018/api/SeaFood/SearchSeafood?nameSeaFood=${nameProduct}&pageIndex=${pageIndex}&pageSize=${pageSize}`
  );
}

async function fetchProductData(pageIndex, pageSize) {
  return await axios.get(
    `https://localhost:7018/api/SeaFood?pageNumber=${pageIndex}&pageSize=${pageSize}`
  );
}

async function fetchFiterSeaFoodData(token) {
  return await axios.get(
    `https://localhost:7018/api/Type/GetTypes?token=${token}`
  );
}

async function fetchSearchProductByTypeData(nameType, pageIndex, pageSize) {
  return await axios.post(
    `https://localhost:7018/api/SeaFood/SearchSeafoodByType?nameSeaFood=${nameType}&pageIndex=${pageIndex}&pageSize=${pageSize}`
  );
}
...
async function fetchAddProduct(token) {
  ...
  var primaryImage = ["imageInput"]; // Lấy tên ảnh chính từ dataset
  var seafoodImagesprimary = [];
  var seafoodImages = [];
  var imageInputs = [
    "imageInputFirstChild",
    "imageInputSecondChild",
    "imageInputThirdChild",
    "imageInputFourthChild",
    "imageInputFiveChild",
  ];

  const uploadImagePromises = primaryImage.map(async (id) => {
    ...
    if (imageName) {
      ...
      formData.append("idFolder", "1nSplZRyY7PZB1617yBC3ejoaeS3ajS3d");

      try {
        const response = await fetch(
          "https://localhost:7018/api/ManagerSeaFood/UploadImgProduct",
          {
            method: "POST",
            body: formData,
          }
        ...
        seafoodImagesprimary.push({ nameImage: linkImg });
      } catch (error) {
        console.error("Error:", error);
  ...
  // Tải lên các ảnh và chờ hoàn tất
  ...
  const requestBody = {
    ...
    quantity: quantity,
    primaryImage: seafoodImagesprimary[0].nameImage,
    seafoodImages: seafoodImages,
  };

  return await axios.post(
    `https://localhost:7018/api/ManagerSeaFood/AddSeaFood?token=${token}`,
    requestBody
  );
}
...
async function fetchProductDetail(id) {
  return await axios.get(
    `https://localhost:7018/api/SeaFood/SeaFoodDetail?id=${id}`
  );
}
...
async function fetchUpdateProduct(id) {
  ...
  var unit = document.getElementById("product_detail--unit").value;
  var origin = document.getElementById("product_detail--origin").value;
  var expirationDate = document.getElementById(
    "product_detail--preserve"
  ).value;
  var price = document.getElementById("product_detail--price").value;
  var quantity = document.getElementById("product_detail--amount").value;
  var selectVoucher = document.getElementById("selectVoucher");
  ...
  var imageInputs = [
    "selectedImage",
    "selectedImageFirstChild",
    "selectedImageSecondChild",
    "selectedImageThirdChild",
    "selectedImageFourthChild",
    "selectedImageFiveChild",
  ];
  ...
  return await axios.put(
    `https://localhost:7018/api/ManagerSeaFood/UpdateSeaFood?idSeaFood=${id}&token=${token}`,
    requestBody
  );
}
...
async function fetchDeleteProduct(token, id) {
  return await axios.delete(
    `https://localhost:7018/api/ManagerSeaFood/DeleteSeaFood?token=${token}&seafoodId=${id}`
  );
}
...
function renderProductPage(data) {
  ...
  var currentProductPage =
    parseInt(localStorage.getItem("currentProductPage")) || 1;
            ...
            <tr>
                ...
                <td class="popup_trigger--product" onclick="togglePopup(event,'popup_content--product')">
                    ...
                    <div class="popup_content--product d-flex d-none flex-column">
                        <a href="#" class="popup-option" onclick="detailProduct(event,${product.id
      })">Detail</a>
                        <a href="#" class="popup-option" onclick="deleteProduct(event,${product.id
      })">Delete</a>
                    </div>
                </td>
            </tr>`;
...
function renderProductDetail(productData) {
  ...
  document.getElementById("product_detail--instruct").value =
    productData.instruct;
  document.getElementById("product_detail--unit").value = productData.unit;
  document.getElementById("product_detail--amount").value =
    productData.quantity;
  document.getElementById("product_detail--origin").value = productData.origin;
  document.getElementById("product_detail--preserve").value =
  ...
  //   "selectedImage"
  // ).src = `../assets/images/product/${productData.primaryImage}`;
  var imageInputs = [
    "selectedImageFirstChild",
    "selectedImageSecondChild",
    "selectedImageThirdChild",
  ...
  document.getElementById("selectedImage").src = productData.primaryImage;

  // if (productData.seaFoodImages) {
  productData.seaFoodImages.forEach((image, index) => {
    if (index < imageInputs.length) {
      document.getElementById(imageInputs[index]).src = image.nameImage;
    }
  });
  // }
  // imageInputs.forEach(id => {
...
function renderProductPagination(
  ...
  var htmlContent = "";
  let onClickFunction =
    localStorage.getItem("searchProductStatus") == "True"
      ? "searchProduct"
      : localStorage.getItem("searchProductByType") !== ""
        ? "getProductByType"
        : "handleProductsPage";
  ...
  htmlContent += `<li class="page_item--Product">
                    ...
                    <li class="page_item--Product">
                        <a class="page-link" href="#" onclick="periousProductPage(${lastProductPage
      ? currentProductPage - 4 - residual
      : currentProductPage - 4
    })">
                            <i class="fa fa-angle-left"></i>
                        </a>
                    </li>`;
  ...
  for (
    let page = lastProductPage ? totalPage - residual : currentProductPage;
    page <= totalPage && page < currentProductPage + 4;
    page++
  ) {
  ...
  htmlContent += `<li class="page_item--product">
                        <a class="page-link" href="#" onclick="nextProductPage(${currentProductPage + 4
    },${totalPage})">
                            <i class="fa fa-angle-right"></i>
                        </a>
                    </li>
  ...
  localStorage.setItem(
    "currentProductPage",
    lastProductPage ? currentProductPage - residual : currentProductPage
  );
}
...
// -------------------- Blog
localStorage.setItem("currentBlogPage", 1);
localStorage.setItem("lastBlogPage", "False");
localStorage.setItem("searchBlogStatus", "False");
localStorage.setItem("searchBlogByType", "");



async function fetchBlogData(pageIndex, pageSize) {
  return await axios.get(
    `https://localhost:7018/api/Blog/getBlogs?pageNumber=${pageIndex}&pageSize=${pageSize}`
  );
}

function nextBlogPage(page, totalPage) {
  if (page > totalPage) {
    return;
  } else {
    localStorage.setItem("currentBlogPage", page);
    if (localStorage.getItem("searchBlogStatus") == "False") {
      handleBlogsPage(page, pageSizeDefault);
    } else {
      searchBlog(page, pageSizeDefault);
    }
  }
}

function periousBlogPage(page) {
  if (page < 1) {
    return;
  } else {
    localStorage.setItem("currentBlogPage", page);
...
function lastBlogPage(page) {
  localStorage.setItem("lastBlogPage", "True");
  localStorage.setItem("currentBlogPage", page);
...
function renderBlogPagination(
  ...
  var lastBlogPage = localStorage.getItem("lastBlogPage") === "True";
  var updateColorPage = localStorage.getItem("updateBlogColor");
  var residual = lastBlogPage
    ? totalPage % 4 > 0
      ? (totalPage % 4) - 1
      : (totalPage % 4) + 3
    : 0;
  var htmlContent = "";
  let onClickFunction =
    localStorage.getItem("searchBlogStatus") == "True"
      ? "searchBlog"
      : localStorage.getItem("searchBlogByType") !== ""
        ? "getBlogByType"
        : "handleBlogsPage";

  htmlContent += `<li class="page_item--Blog">
                        <a class="page-link" href="#" onclick="firstBlogPage(${1})">
                            <i class="fa-solid fa-angles-left"></i>
                        </a>
                    </li>
                    <li class="page_item--Blog">
                        <a class="page-link" href="#" onclick="periousBlogPage(${lastBlogPage
      ? currentBlogPage - 4 - residual
      : currentBlogPage - 4
    })">
                            <i class="fa fa-angle-left"></i>
                        </a>
  ...
  for (
    let page = lastBlogPage ? totalPage - residual : currentBlogPage;
    page <= totalPage && page < currentBlogPage + 4;
    page++
  ) {
    htmlContent += `<li class="page_item--Blog ${page == updateColorPage ? "active" : ""
      }">
                            <a class="page-link" onclick="${onClickFunction}(${page},${pageSizeDefault})" href="#" value="${page}">${page}</a>
                        </li>`;
  }
  ...
  htmlContent += `<li class="page_item--Blog">
                        <a class="page-link" href="#" onclick="nextBlogPage(${currentBlogPage + 4
    },${totalPage})">
                            <i class="fa fa-angle-right"></i>
                        </a>
                    </li>
                    <li class="page_item--Blog">
                        <a class="page-link" href="#" onclick="lastBlogPage(${totalPage})">
                            <i class="fa-solid fa-angles-right"></i>
                        </a>
                    </li>`;
  ...
  localStorage.setItem(
    "currentBlogPage",
    lastBlogPage ? currentBlogPage - residual : currentBlogPage
  );
}

function renderBlogPage(data) {
  const totalPage = Math.ceil(parseInt(data.length) / pageSizeDefault);
  const tableBody = document.getElementById("blogTableBody");
  const paginationLoad = document.getElementById("pagination_Blog--nav");
  const totalBlogText = document.getElementById("total_blogs");
  var currentBlogPage = parseInt(localStorage.getItem("currentBlogPage")) || 1;
  ...
  if (!data || data.length === 0) {
    tableBody.innerHTML =
      '<div class="none_data">Không có dữ liệu</div>';
    totalBlogText.innerHTML = "0";
    return;
  }

  data.forEach((blog) => {
    const date = new Date(blog.publishedDate).toLocaleDateString("vi-VN", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit"
    });
    const row = `
                <div class="col-6 col-sm-6 col-md-3 p-2" onclick=getDetailBlog('${blog.id}')>
                        <div class="blog_item d-flex flex-column p-2 border">
                            <div class="blog_item--image zoom-image w-100">
                                <img src="https://drive.google.com/thumbnail?id=${blog.thumbnail}&sz=w1000" alt="Google Drive Image">
                            </div>
                            <div class="blog_item--infor d-flex justify-content-between my-2">
                                <div class="blog_infor--date">${date}</div>
                                <div class="blog_infor--group d-flex">
                                    <div class="blog_group--view">
                                        <span>${blog.views}</span>
                                        <i class="fa-regular fa-eye"></i>
                                    </div>
                                    <div class="blog_group--like ms-2">
  ...
  totalBlogText.innerHTML = data.length;
  // renderBlogPagination(currentBlogPage, totalPage, paginationLoad);
}

async function handleBlogsPage(pageIndex, pageSize) {
  localStorage.setItem("updateBlogColor", pageIndex);
  localStorage.setItem("searchBlogStatus", "False");

  await fetchBlogData(pageIndex, pageSize)
    .then((response) => {
      renderBlogPage(response.data);
    })
    .catch((error) => console.error("Error:", error));
}
  ...
  if (imageInput && imageInput.files.length > 0) {
    ...
    try {
      const uploadImgResponse = await axios.post(
        "https://localhost:7018/api/ManagerBlog/UploadBlogImg?idFolder=1dNjkxKq_8deqs84jh8OGnD1-3H2C6UfR",
        formData,
        {
          headers: {
      ...
      // Đẩy nội dung lên
      const imageId = uploadImgResponse.data;
      var title = document.querySelector("#blog_detail--name").value;
      var content = tinymce.activeEditor.getContent("textPalce");
      ...
      await axios.post(`https://localhost:7018/api/ManagerBlog/UploadBlogContent?token=${token}`, blog)
      alert("Add blog successfully");
      resetForm();
      toggleAddBlogForm();
      handleBlogsPage(1, 100);
...
function renderDetailBlog(data, idBlog) {

  const selectedBlogImage = document.getElementById("selectedBlogImage");
  selectedBlogImage.src = `https://drive.google.com/thumbnail?id=${data.thumbnail}&sz=w800`;
  const blogTitleInput = document.getElementById("blog_detail--name");
  blogTitleInput.value = data.title;

  tinymce.get("textPalce").setContent(data.content);
  toggleUpdateBlogForm(idBlog)

}

async function getDetailBlog(idBlog) {
  try {
    const response = await axios.get(`https://localhost:7018/api/Blog/getBlogDetail?idBlog=${idBlog}`);
    renderDetailBlog(response.data, idBlog);
  } catch (error) {
    console.error("Error fetching blog details:", error);
  }
}

async function updateBlog(idBlog) {
  // Xóa ảnh hiện tại
  const response = await axios.get(`https://localhost:7018/api/Blog/getBlogDetail?idBlog=${idBlog}`);
  var idOldImg = response.data.thumbnail;
  await axios.delete(`https://localhost:7018/api/ManagerBlog/DeleteImage?idImg=${idOldImg}`);
  ...
  if (imageInput && imageInput.files.length > 0) {
    const file = imageInput.files[0];
    const formData = new FormData();
    const idFolder = "1dNjkxKq_8deqs84jh8OGnD1-3H2C6UfR";
    formData.append("file", file);

    try {
      const uploadImgResponse = await axios.post(
        `https://localhost:7018/api/ManagerBlog/UploadBlogImg?idFolder=${idFolder}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      var idNewImg = uploadImgResponse.data;
      var title = document.querySelector("#blog_detail--name").value;
      var content = tinymce.activeEditor.getContent("textPlace");
      ...
      var responseUpdate = await axios.post(`https://localhost:7018/api/ManagerBlog/UpdateBlogContent?token=${token}&idBlog=${idBlog}`,
        blog,
        {
          headers: {
